locals {
  ...
  function_name = "lambda-email-function"
  tags = {
...
module "lambda_email_function" {
  ...
  store_on_s3              = true
  s3_bucket                = "ons-dp-sandbox-encrypted-datasets"
  ...
  environment_variables = {
    ...
    bucket_name = "ons-dp-sandbox-encrypted-datasets"
    download_url = "download.dp.aws.onsdigital.uk/downloads-new"
    ...
    Statement = [
      {
        ...
        Resource = "arn:aws:ses:eu-west-2:337289154253:identity/florence@dp.aws.onsdigital.uk"
      }
    ]
  })
}
...
module "s3_bucket" {
  ...
  bucket        = "dp-sandbox-${local.function_name}"
  force_destroy = true
